...
class BLS_Query:
    ...
    def qcewGetAreaData(self, area):
        urlPath = "http://api.bls.gov/cew/data/api/[YEAR]/[QTR]/area/[AREA].csv"
    ...
    def qcewGetIndustryData(self, industry):
        urlPath = "http://api.bls.gov/cew/data/api/[YEAR]/[QTR]/industry/[IND].csv"
        urlPath = urlPath.replace("[YEAR]",self.year)
        urlPath = urlPath.replace("[QTR]",self.qtr.lower())
    ...
    def qcewGetSizeData(self, size):
        urlPath = "http://api.bls.gov/cew/data/api/[YEAR]/1/size/[SIZE].csv"
        urlPath = urlPath.replace("[YEAR]",self.year)
    ...
    def readIndCodes(self):
        try:
            ind_data = pd.read_csv(self.ind_titles_path, sep='#')
        except:
